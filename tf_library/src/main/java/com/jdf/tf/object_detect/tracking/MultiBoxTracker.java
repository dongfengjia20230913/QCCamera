...
package com.jdf.tf.object_detect.tracking;
...
public class MultiBoxTracker {
  ...
  public synchronized void trackResults(final List<Classifier.Recognition> results, final long timestamp) {
    JLog.i(JLog.TAG_DETECTION,"Processing %d results from %d", results.size(), timestamp);
  ...
  private void processResults(final List<Classifier.Recognition> results) {
    ...
    for (final Classifier.Recognition result : results) {
      ...
      if (detectionFrameRect.width() < MIN_SIZE || detectionFrameRect.height() < MIN_SIZE) {
        JLog.d(TAG,"Degenerate rectangle! " + detectionFrameRect);
        continue;
      }
    ...
    if (rectsToTrack.isEmpty()) {
      JLog.v(TAG,"Nothing to track, aborting.");
      return;
    }
